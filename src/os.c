...
static void * cpu_routine(void * args) {
	...
	while (1) {
		...
		}else if (proc->code ==NULL ||proc->pc == proc->code->size) { // ktra (NULL) code đã hoàn thành
...
static void * ld_routine(void * args) {
	...
	int active_mswp_id=((struct mmpaging_ld_args *)args)->active_mswp_id; //add
#else
	...
	while (i < num_processes) {
		...
		proc->active_mswp_id=active_mswp_id; //Thêm sử dụng chỉ số bộ nhớ swap nào trong mswp
#endif
...
int main(int argc, char * argv[]) {
	...
	// gán trực tiếp địa chỉ mảng bị sai


	//cấp phát bộ nhớ cho con trỏ mswp để gán từng phần tử
	mm_ld_args->mswp = malloc(PAGING_MAX_MMSWP * sizeof(struct memphy_struct*));


	for (i = 0; i < PAGING_MAX_MMSWP; i++) {
		mm_ld_args->mswp[i] = &mswp[i];  
	}
	for (i = 0; i < PAGING_MAX_MMSWP; i++){
		if(memswpsz[i]>=256) {
			mm_ld_args->active_mswp = mm_ld_args->mswp[i];
    		mm_ld_args->active_mswp_id = i;
			break;
		}
	}
#endif

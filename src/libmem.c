...
int __alloc(struct pcb_t *caller, int vmaid, int rgid, int size, int *alloc_addr)
{
  ...
  pthread_mutex_lock(&mmvm_lock);//tránh xung đột giữa các luồng
  // kiểm tra rgid có hợp lệ không
  if (rgid < 0 || rgid > PAGING_MAX_SYMTBL_SZ)
  {
  ...
  //cấp phát bộ nhớ cho vùng mới từ old_sbrk đến old_sbrk+size
  ...
  *alloc_addr = old_sbrk;
  ...
  //ếu còn không gian trống sau khi cấp phát, thêm vào freerg_list
...
int __free(struct pcb_t *caller, int vmaid, int rgid)
{
  ...
  //khóa mutex để tránh xung đột giữa các luồng
  ...
  unsigned long start = rg->rg_start;
  unsigned long end   = rg->rg_end;
  ...
  //Thêm vùng nhớ đã giải phong vào danh sách vùng nhớ trống
  ...
  new_rg->rg_start = start;
  new_rg->rg_end = end;
  new_rg->rg_next = NULL;

  enlist_vm_freerg_list(caller->mm, new_rg);
  //Đặt lại rg_start và rg_end về 0
 ...
 */
//Trả lại vùng nhớ cho bộ nhớ vật lý
